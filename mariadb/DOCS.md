# Home Assistant Custom App: In-memory MariaDB
...
> trying to minimize your SD-card's wear, give this app a try.
>
...
> content when the app starts again**. The exported database (an SQL file generated by
...
> failure, when the app is restarted, it will import the last known exported
...
>
> The other notable difference is that the app purges old statistics data from
...
You can use this app to install an _**in-memory**_ MariaDB, which is an
...
1. In the **...** menu at the top right corner click **Repositories**, add
   ...
   repository.
1. Find the "In-memory MariaDB" app and click it. If it doesn't show up, wait
   until HA refreshes the information about the app, or click **Check for
...
If you update or restart the app, please stop HA core to avoid error messages
...
Or you can execute these commands step by step manually to update the app:
- \> ha core stop
- \> ha apps update 45207088_mariadb --backup
- \> ha core start

## App configuration

The MariaDB server app can be tweaked to your likings. This section describes
each of the app configuration options.
...
This app uses tmpfs to store MariaDB databases in-memory. This section
...
> DBeaver, BeeKeeper-Studio. Or hardcore users can use Portainer app to get a
> console to this app's container (or without Portainer app SSH into the
...
when the app starts again. The exported database (an SQL file generated by
sqldump) is gzip-ed before written to the storage to minimize SD-card wear.
...
- the database dump is **gzip-ed** before written to the storage to minimize
  SD-card wear
- after a power failure, when the app is restarted, it will import the last
  known exported database content

If enabled (default) the app will
...
If disabled the app will delete any previously saved database content when
started (restarted).
...
because exporting database content is part of the app's backup.
...
can be acessed with eg. the Samba share app. This folder is also part of the
full backup of Home Assistant, so without moving these exported old data
anywhere, they will be part of the Home Assistant backup (not the app's
backup, these are deleted from the database, these are archive values, not
belonging to the app anymore).
...
is here to be compatible with the official app.
...
you entered in the app configuration.

**Note:** The `45207088-mariadb` is the Hostname displayed on the app's Info
tab.
...
[official_app]: https://github.com/home-assistant/addons/tree/master/mariadb
